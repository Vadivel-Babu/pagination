...
window.addEventListener("load", () => {
   display = data.slice((count -1)*10, (count * 10));
 display.forEach((item) => {
  let table = `<tr> <td>${item.id}</td> <td>${item.name}</td> <td>${item.email}</td> </tr>`
  cell.innerHTML += table
 })
});

function selectPage(element){
  if(element.target.innerText === 'First'){
    count = 1;
  }else if(element.target.innerText === 'Last'){
    count = 10;
  }else if(element.target.innerText === 'Next'){
    if(count === 10){
      return;
    }
    count += 1;
  }else if(element.target.innerText === 'Previous'){
    if(count === 1){
      return;
    }
    count -= 1;
  } else {
    count = +(element.target.innerText);
  }
  for (let i = 0; i < btn.length; i++) {
    if(btn[i].innerText == count){
      btn[i].classList.add('active');      
    }else if(count == 10 && btn[i].innerText == 'Last'){
      btn[i].classList.add('active'); 
    }else if(count == 1 && btn[i].innerText == 'First'){
      btn[i].classList.add('active'); 
    } else{
...
document.querySelectorAll('.btn-1').forEach((e) => {
  e.addEventListener('click',selectPage);
})
